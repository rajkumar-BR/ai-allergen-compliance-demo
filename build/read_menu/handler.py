...
def _handle_list_restaurants(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Fail closed: no partial list is returned, but log the cause so a
        # 500 here is diagnosable instead of silent (it previously wasn't).
        logger.exception("GET /restaurants scan failed (table=%s)", TABLE_NAME)
