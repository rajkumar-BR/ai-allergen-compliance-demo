...
3. **Upload timeout on a moderately-sized menu.** A test PDF with 8 dishes reliably returned a bare `Internal Server Error` with zero application-level log output on upload. Diagnostic logging (`upload_menu`) isolated the cause: `upload_menu` originally ran the per-dish pipeline through an uncapped `ThreadPoolExecutor` — every dish's two Bedrock calls fired at once, which this shared demo account's on-demand throughput quota throttled immediately (`ThrottlingException`), and the retries that triggered inside `bedrock_service` ran out the function's time budget before any dish finished. Switched to sequential processing (matching `dish_mutations`' already-working seed route) — which still measured ~26-29s for 8 dishes with zero margin against API Gateway's hard, non-configurable 30s integration timeout, since each dish makes two Bedrock calls at roughly 1.5-3s apiece even with no throttling at all. This is a real capacity limit of the current synchronous design, not something a bigger Lambda timeout can fix (API Gateway's ceiling doesn't move) — see the upload-timeout row below.
...
| A menu of more than ~4-5 dishes may time out on upload | `upload_menu` processes dishes sequentially (two Bedrock calls each) against API Gateway's hard 30s integration timeout - measured at 26-29s for 8 dishes with no margin for a single retry | Same fix as above: async processing has no request/response round-trip to time out |
