// Filled in by config.js (generated at deploy time from the Terraform
// menu_api_invoke_url output) - see DEPLOY_GUIDE.md. The frontend is served
// from Amplify Hosting, a different origin than the API Gateway endpoint, so
// every API call is an absolute URL rather than a same-origin relative path.
...
// admin-only: change the admin login password (stored in Secrets Manager,
// see build/auth/handler.py's POST /auth/change-password).
function changePassword() {
  document.getElementById("modalTitle").textContent = "Change Password";
  document.getElementById("modalBody").innerHTML = `
    <div id="changePasswordError" class="muted" style="color:#c0392b; display:none;"></div>
    <div id="changePasswordSuccess" class="muted" style="color:#2e7d32; display:none;"></div>
    <label>Current password</label>
    <input type="password" class="form-control" id="currentPassword" />
    <label class="mt-2">New password</label>
    <input type="password" class="form-control" id="newPassword" />
    <div class="modal-actions">
      <button class="btn btn-primary" id="changePasswordSubmitBtn">Update password</button>
    </div>
  `;
  document.getElementById("editModal").classList.remove("hidden");
...
async function loadCategories() {
  const data = await api("/allergen-categories");
...
async function loadItems() {
  try {
    const data = await api(`/menus/${MENU_ID}`);
    allItems = data.items || [];
  } catch (err) {
...
function openModal(itemId) {
  ...
  document.getElementById("saveReviewBtn").onclick = async () => {
    ...
    await api(`/menus/${MENU_ID}/items/${itemId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", "Authorization": `Bearer ${adminToken}` },
...
document.getElementById("clearBtn").addEventListener("click", async () => {
  ...
  try {
    const result = await api(`/menus/${MENU_ID}`, {
      method: "DELETE",
      headers: { "Authorization": `Bearer ${adminToken}` },
    });
    activeFilters.clear();
...
document.getElementById("uploadBtn").addEventListener("click", async () => {
  ...
  try {
    const result = await api(`/menus/${MENU_ID}/upload`, {
      method: "POST",
      headers: { "Authorization": `Bearer ${adminToken}` },
      body: formData,
    });
